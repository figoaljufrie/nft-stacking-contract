// test/nft-staking-test.ts

import { expect } from "chai";
import hre from "hardhat";
import type { NFTStaking } from "../types/ethers-contracts/NFTStaking.js";

// Hardhat provides describe/it/beforeEach automatically via Mocha,
// so we DO NOT import them from "mocha"
...
describe("NFTStaking Contract", function () {
  ...
  beforeEach(async () => {
    // Access ethers from Hardhat runtime
  ...
  it("should fail when staking 0", async () => {
    const { viem } = await hre.network.connect("generic");
    await viem.assertions.revertWith(
      nftStaking.connect(alice).stake(0),
